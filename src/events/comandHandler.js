...
const iltifatlar = [
    'Gözlerin öyle güzel parlıyor ki karanlığı aydınlatan lamba gibi',
    ...
    'İğrenç İnsansın!',
    'Ben gök yüzüne bakmıştım ama gözlerinin içinde evreni mi buldum',
...
module.exports = async (message) => {
    if (!message.guild || message.author.bot) return
      const owner = client.users.cache.get("689977464663179309");
    ...
    if (message.channel.id === config.channels.chat) {
      ...
      const ownerr = client.users.cache.get("689977464663179309");
    const afkembed = new MessageEmbed()
        .setColor(message.member.displayHexColor)
        .setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true, size: 2048 }))
        .setFooter("Rahibe", ownerr.avatarURL({ dynamic: true }))
        .setTimestamp()
