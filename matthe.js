    ...
    files.forEach(f => {
        readdir("./src/commands/" + f, (err2, files2) => {
            ...
            files2.forEach(file => {
                ...
                console.log(`[Rahibe Command] ${prop.name} yüklendi!`);
...
readdir("./src/events", (err, files) => {
    ...
    files.filter((file) => file.endsWith(".js")).forEach((file) => {
        ...
        console.log(`[Rahibe-EVENT] ${prop.conf.name} yüklendi!`);
    });
});

client.on("message", async message => {
    if (message.content === "!buton-rolss" && message.author.id === config.bot.owner) {
...
`,
    ...
    if (message.content === "!buton-bilgiss" && message.author.id === config.bot.owner) {
...
client.login("NzQyNDY3NDczODI3MTY4Mzc3.XzGiuA.TJtcO00CATAs0YefQbUo317xYRQ").then(x => console.log(`Bot ${client.user.username} olarak giriş yaptı!`)).catch(err => console.log(`Bot Giriş yapamadı sebep: ${err}`));
